...
* 1) define the data buffer
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_buffer.
             INCLUDE TYPE   zsubcontracting AS data.
...
ENDCLASS.
CLASS lhc_zsubconcds DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

*    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
*      IMPORTING keys REQUEST requested_authorizations FOR zsubconcds RESULT result.

    METHODS create FOR MODIFY
      IMPORTING gdata FOR CREATE zsubconcds.

    METHODS update FOR MODIFY
      IMPORTING entities FOR UPDATE zsubconcds.

    METHODS delete FOR MODIFY
      IMPORTING keys FOR DELETE zsubconcds.

    METHODS read FOR READ
      IMPORTING keys FOR READ zsubconcds RESULT result.

*    METHODS lock FOR LOCK
*      IMPORTING keys FOR LOCK zsubconcds.

ENDCLASS.

CLASS lhc_zsubconcds IMPLEMENTATION.
...
*  ENDMETHOD.
  ...
  METHOD create.
    DATA: number TYPE cl_numberrange_runtime=>nr_number,
          mapgen LIKE LINE OF mapped-zsubconcds.
    ...
    LOOP AT gdata INTO DATA(dt) ."GROUP BY ( key1 = dt-plant key2 = dt-invoiceno key3 = dt-podocno key4 = dt-supplier  ).
      ...
      SELECT SINGLE * FROM zsubcontracting  WHERE plant = @dt-plant
      AND supplier = @dt-supplier AND billingdocument = @dt-billingdocument AND cancelledind NE 'X' INTO @DATA(wg).
      IF sy-subrc NE 0.
        ...
        IF dt-isclosed = 'X'.
          ...
          dt-closedtime = |{ sy-timlo+0(2) }:{ sy-timlo+2(2) }:{ sy-timlo+4(2) }|..
        ENDIF.
...
*              append tgdt to mapped-zgenpurchasecds.
        IF dt-%cid IS NOT INITIAL.
          ...
          APPEND mapgen TO mapped-zsubconcds.
...
CLASS lsc_zsubconcds DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_zsubconcds IMPLEMENTATION.
  ...
  METHOD save.
    DATA lt_data TYPE STANDARD TABLE OF zsubcontracting.
    DATA: tdata1 TYPE STANDARD TABLE OF zsubconcds.
    MOVE-CORRESPONDING lhc_buffer=>mt_data TO lt_data.
*    IF lt_data IS NOT INITIAL.
    TRY.
        INSERT zsubcontracting
         FROM TABLE @lt_data.
      CATCH cx_sy_open_sql_db.
        MODIFY zsubcontracting FROM TABLE @lt_data.
    ENDTRY.
  ENDMETHOD.
